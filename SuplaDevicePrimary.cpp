...
bool SuplaDeviceClass::begin(IPAddress *local_ip, char GUID[SUPLA_GUID_SIZE], uint8_t mac[6], const char *Server,
	...
	setString(Params.reg_dev.SoftVer, "2.0.2", SUPLA_SOFTVER_MAXSIZE);
...
int SuplaDeviceClass::addRelayButton(int relayPin, int buttonPin, int type_button, int flag, bool hiIsLo, _supla_int_t DurationMS, _supla_int_t functions) {
	...
	if ( relayPin != -1 ) {
		...
		} else if ( flag == RELAY_FLAG_ON ) {
			pinMode(relayPin, OUTPUT);
			suplaDigitalWrite(Params.reg_dev.channels[c].Number, relayPin, _HI); //digitalWrite(relayPin, HIGH);
		} else if ( flag == RELAY_FLAG_OFF ) {
			pinMode(relayPin, OUTPUT);
			suplaDigitalWrite(Params.reg_dev.channels[c].Number, relayPin, _LO); //digitalWrite(relayPin, LOW);
		} else {	
			pinMode(relayPin, OUTPUT); 
	 ...
	 if ( channel->Type == SUPLA_CHANNELTYPE_RELAY ){
			 ...
			 if ( pin->start == 0 ) {
				 if ( pin->flag == RELAY_FLAG_RESTORE && Params.cb.read_supla_relay_state != 0) {
					...
					if (val == 0 || val == 1) {
						//channelValueChanged(channel->Number, val == _LO ? 1 : 0);	
						channelSetValue(channel->Number, val == _HI ? 1 : 0, 0);
